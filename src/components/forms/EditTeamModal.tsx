...
import { updateTeam } from "@/services/teams/team-service";
...
export default function EditTeamModal({
  ...
  useEffect(() => {
    if (team) {
      ...
      setValue("short_name", team.short_name || "");
      setValue("city", team.city || "");
      setValue("manager", team.manager || "");
      setImagePreview(team.badge_url || null);
      setImageFile(null);
    }
  }, [team, setValue]);
  ...
  const handleImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const sanitizedFile = new File(
      [file],
      `${cleanName}_${Date.now()}.${cleanExtension}`,
      { type: file.type }
    );
  ...
  const onSubmitForm = async (data: CreateTeamFormData) => {
    try {
      ...
      const message =
        error instanceof Error ? error.message : "Erro ao atualizar a equipe.";
      alert(message);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 bg-black/80 backdrop-blur-sm">
      <div className="bg-brand-dark border border-slate-850 rounded-2xl w-full max-w-lg mx-auto max-h-[85vh] overflow-y-auto shadow-2xl flex flex-col scrollbar-none">
        ...
        <div className="p-4 sm:p-5 border-b border-slate-800/60 flex items-center justify-between sticky top-0 bg-brand-dark z-10">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-base font-bold text-brand-textPrimary">Editar Equipe</h2>
          </div>
          <button
            type="button"
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit(onSubmitForm)} className="p-4 sm:p-6 space-y-5 flex-1">
          ...
          <div className="flex flex-col sm:flex-row gap-5 items-center bg-slate-900/30 p-4 rounded-xl border border-slate-800/50">
            <div className="flex flex-col items-center">
              ...
              >
                {imagePreview ? (
                  <img src={imagePreview} alt="Preview do Escudo" className="w-full h-full object-cover" />
                ) : (
            ...
            <div className="flex-1 w-full space-y-3">
              <div className="space-y-1">
                ...
                <input
                  ...
                  disabled={loading}
                  {...register("name")}
                  className="w-full bg-slate-900 border border-slate-800 rounded-xl px-3.5 md:py-2 py-2.5 text-xs text-brand-textPrimary placeholder-slate-600 focus:outline-none focus:border-brand-accent"
                />
              ...
              <div className="space-y-1">
                ...
                <input
                  ...
                  disabled={loading}
                  {...register("short_name")}
                  className="w-28 bg-slate-900 border border-slate-800 rounded-xl px-3.5 md:py-2 py-2.5 text-xs text-brand-textPrimary text-center placeholder-slate-600 focus:outline-none focus:border-brand-accent"
                />
              </div>
            </div>
          </div>
