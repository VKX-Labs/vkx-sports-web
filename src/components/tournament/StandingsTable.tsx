...
export function StandingsTable({
  ...
  return (
    <div className="w-full bg-zinc-900/80 rounded-xl border border-zinc-800/80 overflow-hidden backdrop-blur-md">
      ...
      <div className="md:hidden flex items-center gap-1.5 px-4 py-2 bg-emerald-500/5 border-b border-zinc-800/60">
        <span className="text-[10px] text-emerald-400/80 font-medium animate-pulse">→</span>
        <span className="text-[10px] text-zinc-500">Deslize para ver todas as estatísticas</span>
      </div>

      <div className="relative">
        <div
          className="overflow-x-auto scroller-smooth w-full"
        ...
        >
          <table className="w-full min-w-[640px] text-left text-xs text-zinc-300">
            <thead className="bg-zinc-950/60 text-[10px] text-zinc-400 uppercase font-mono tracking-wider border-b border-zinc-800/60">
              <tr>
                <th className="sticky left-0 z-20 bg-zinc-950 py-2.5 px-2 md:px-3 text-center w-10">Pos</th>
                <th className="sticky left-10 z-20 bg-zinc-950 py-2.5 px-2 md:px-3 min-w-[140px]">Equipe</th>
              ...
              {standings.map((team) => {
                ...
                return (
                  <tr key={team.team_id} className="hover:bg-zinc-800/40 transition-colors group">
                    <td className="sticky left-0 z-10 bg-zinc-900 group-hover:bg-zinc-800/70 transition-colors py-2.5 px-2 md:px-3 text-center font-mono text-xs font-bold">
                      <span
                    ...
                    <td className="sticky left-10 z-10 bg-zinc-900 group-hover:bg-zinc-800/70 transition-colors py-2.5 px-2 md:px-3 font-semibold text-zinc-100 border-r border-zinc-800/60">
                      <div className="flex items-center gap-2.5">
                        ...
                        <span className="truncate max-w-[120px] sm:max-w-none text-xs md:text-sm">
                          {team.team_name}
                        </span>
                      </div>
                    </td>
        ...
        <div className="pointer-events-none absolute inset-y-0 right-0 w-6 md:w-8 bg-gradient-to-l from-zinc-900 to-transparent" aria-hidden="true" />
      </div>
    </div>
  );
}