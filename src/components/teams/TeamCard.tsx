...
import { Shield, Users, MapPin, Settings2, ShieldAlert } from "lucide-react";
  ...
  return (
    <div className="bg-[#111827]/40 border border-slate-800 rounded-2xl p-5 flex flex-col justify-between transition hover:border-slate-700 hover:bg-[#111827]/70 group">
      ...
      <div className="flex items-start gap-4">
        {/* Renderização do Escudo ou Placeholder */}
        ...
        <div className="space-y-1 truncate">
          ...
          <p className="text-xs text-slate-400 flex items-center gap-1">
            ...
            {team.city || "Não informada"}{team.state ? ` - ${team.state}` : ""}
          </p>

          <p className="text-xs text-slate-400 flex items-center gap-1 pt-1">
            <Users className="w-3.5 h-3.5 text-emerald-500" />
            <span className="font-semibold text-slate-300">{playerLength}</span> {playerLength === 1 ? "jogador" : "jogadores"}
          </p>
        </div>
      </div>

      {/* Ações Inferiores Separadas */}
